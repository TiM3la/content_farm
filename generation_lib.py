...
class Main_DB:
    ...
    def make_music_table(self):
        try:
            print(f'[Процесс...] Создаем таблицу Music')
            self.base.execute("create table if not exists Music (id integer primary key, title varchar, date_create TIMESTAMP_S, duration integer, date_last_use TIMESTAMP_S, use_number integer, link_yd varchar)")
            return '[ОК] Таблица Music создана'
        except Exception as e:
            return f'[!] Ошибка! Таблица Music не создана. {e}'

    def load_music(self):
        try:
            print(f'[Процесс...] Загружаем музыку')
            for item in self.y.listdir("app:/music"):
                if item.name.endswith('.mp3'):
                    id, title = item.name.rstrip('.mp3').split('_')
                    result = self.base.execute("select * from Music where id = ?", [id]).fetchall()
                    if not bool(result):
                        self.base.execute(
                            'insert into Music (id, title, date_create, link_yd, use_number) values(?, ?,  NOW()::TIMESTAMP::TIMESTAMP_S, ?, ?)',
                            [int(id), title, f'app:/music/{item.name}', 0])
                        print(f'[ОК] Загружена музыка {item.name}')
                    else:
                        print(f'[ОК] Музыка {item.name} была загружена ранее')
                else:
                    print(f'[?] В папке нет музыки')
        except Exception as e:
            print(f'[!] Ошибка! Музыка не загружена. {e}')
    ...
    def make_img_table(self):
        try:
            ...
            self.base.execute("create table if not exists Image (id integer primary key default nextval('seq_image_id'), prompt_id integer REFERENCES Prompt_img(id), date_create TIMESTAMP_S, date_make_video TIMESTAMP_S, date_last_use TIMESTAMP_S, use_number integer, mistery_question varchar, link_yd varchar)")
    ...
    def make_video_table(self):
        try:
            ...
            self.base.execute("create table if not exists Video (id integer primary key default nextval('seq_video_id'), img_id integer REFERENCES Image(id), date_create TIMESTAMP_S, date_last_use TIMESTAMP_S, use_number integer, link_yd varchar)")
            return '[ОК] Таблица Video создана'
        except Exception as e:
            return f'[!] Ошибка! Таблица Video не создана. {e}'

    def make_video_task_table(self):
        try:
    ...
    def make_img(self, client):
        ...
        try:
            img_id = self.base.execute('insert into Image(prompt_id, use_number) values (?, 0) returning id',
                                             [prompt_object['id']])
    ...
    def make_video(self, client):
        ...
        df = self.base.execute('select * from Image where date_make_video is null order by id limit 1').df()
        if df.empty:
        ...
        try:
            video_id = self.base.execute('insert into Video(img_id, use_number) values (?, 0) returning id',
                                       [img_object['id']])
    ...
    def find_clips(self, n):
        video_objects = []
        videos = self.base.execute("select id from Video order by use_number, date_last_use, id limit ?", [n]).fetchall()
        if videos:
            for video in videos:
                df = self.base.execute('select * from Video where id = ?', [video[0]]).df()
                if df.empty:
                    return False
                video_object = df.iloc[0].to_dict()
                video_objects.append(video_object)
                self.base.execute('update Video set date_last_use = NOW()::TIMESTAMP::TIMESTAMP_S, use_number = ? where id = ?', [int(video_object['use_number']) + 1, video_object['id']])
            return video_objects
        else:
            return False

    def find_images(self, n):
        image_objects = []
        images = self.base.execute("select id from Image order by use_number, date_last_use, id limit ?", [n]).fetchall()
        if images:
            for image in images:
                df = self.base.execute('select * from Image where id = ?', [image[0]]).df()
                if df.empty:
                    return False
                image_object = df.iloc[0].to_dict()
                image_objects.append(image_object)
                self.base.execute('update Image set date_last_use = NOW()::TIMESTAMP::TIMESTAMP_S, use_number = ? where id = ?', [int(image_object['use_number']) + 1, image_object['id']])
    ...
    def write_video_task(self, video_task_dict):
        video_task_id = self.base.execute('insert into Video_Task (type, date_create, title, duration, video_num, music_id, voiceover_id, video_list) values (?, NOW()::TIMESTAMP::TIMESTAMP_S, ?, ?, ?, ?, ?, ?) returning id',[
            video_task_dict['type'],
            video_task_dict['title'],
            video_task_dict['duration'],
            video_task_dict['video_num'],
            video_task_dict['music_id'],
            video_task_dict['voiceover_id'],
            video_task_dict['video_list']
        ]).fetchone()[0]
